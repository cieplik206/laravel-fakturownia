...
final readonly class IssueProformaOutcomeProjectionPlanner implements OutcomeProjectionPlanner
{
    public function plan(ProjectionInput $input): ProjectionPlan
    {
        if (! $input->result instanceof IssueInvoiceResult
            || $input->operation->operationType()->value !== IssueProformaOperationFactory::OperationType) {
            throw new InvalidArgumentException('Issue proforma projection planner received an unsupported result.');
        }

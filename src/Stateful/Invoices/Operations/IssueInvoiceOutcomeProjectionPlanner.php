...
final readonly class IssueInvoiceOutcomeProjectionPlanner implements OutcomeProjectionPlanner
{
    public function plan(ProjectionInput $input): ProjectionPlan
    {
        if (! $input->result instanceof IssueInvoiceResult
            || $input->operation->operationType()->value !== IssueInvoiceOperationDefinitionProvider::OperationType) {
            throw new InvalidArgumentException('Issue invoice projection planner received an unsupported result.');
        }
